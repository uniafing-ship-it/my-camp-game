...
func _build_ui() -> void:
	...
	quest_header.add_theme_color_override("font_color", Color(0.84, 0.90, 0.82))
	...
	_quest_reward.add_theme_font_size_override("font_size", 12)
	...
	research_header.add_theme_color_override("font_color", Color(0.84, 0.90, 0.82))
	...
	_save_status.add_theme_color_override("font_color", Color(0.72, 0.78, 0.72))
...
func _layout() -> void:
	...
	_drawer.size = Vector2(width, maxf(300.0, height))
...
func _refresh() -> void:
	...
	for item in _progression.get_research_snapshot():
		...
		if bool(item.get("owned", false)):
			...
			button.text = "%s\n%s · %s" % [str(item.get("name", "")), str(item.get("description", "")), _cost_text(item.get("cost", {}))]
...
func _cost_text(cost: Dictionary) -> String:
	...
	var names := {"wood":"🌲", "stone":"🪨", "food":"🍓", "gold":"🪙"}
